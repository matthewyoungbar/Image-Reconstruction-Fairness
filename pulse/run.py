...
orig = json.load(open('../"Face Evaluation"/face_data.json'))

last = ""
tries = 0
for x in range(len(dataloader)):
    # print(ref_im_name)
    ref_im = dataloader[x][0]
    ref_im_name = dataloader[x][1]
    if last == ref_im_name:
        tries += 1
    else:
        last = ref_im_name
    if(kwargs["save_intermediate"]):
        ...
        analysis = DeepFace.analyze(img_path = out_path / f"{ref_im_name[i]}.png", actions = ['gender', 'race'])
        diff = 0
        img = ref_im_name[0].strip('.png').strip('_0').zfill(5)
        for race in data[image]["race"]:
            diff += abs(analysis["race"][race] - orig[img]["race"][race])
        diff/=len(data[image]["race"])
        if diff >= 12 and tries < 10:
            x-=1
